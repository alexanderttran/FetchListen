...
async function getDownloadStream(videoId, options = {}) {
  ...
  try {
    info = await youtubedl(videoId, { dumpSingleJson: true, noWarnings: true, addHeader: [`X-Forwarded-For:${randomIp}`] });
  } catch (err) {
    extractorArgs = 'youtube:player_client=android,ios;player_skip=webpage,configs';
    info = await youtubedl(videoId, { dumpSingleJson: true, noWarnings: true, addHeader: [`X-Forwarded-For:${randomIp}`], extractorArgs });
  }
