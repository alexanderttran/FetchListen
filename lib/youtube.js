...
async function getVideoInfo(videoId) {
  // Generate a random IPv4 address to help bypass YouTube's IP-based bot blocks on Vercel
  const randomIp = `${Math.floor(Math.random() * 253) + 1}.${Math.floor(Math.random() * 254)}.${Math.floor(Math.random() * 254)}.${Math.floor(Math.random() * 253) + 1}`;

  // Handle Cookies for Vercel Datacenter IPs
  let cookiesFile = null;
  if (process.env.YOUTUBE_COOKIES) {
    cookiesFile = path.join(process.platform === 'win32' ? process.env.TEMP : '/tmp', 'yt_cookies.txt');
    try {
      if (!fs.existsSync(cookiesFile)) fs.writeFileSync(cookiesFile, process.env.YOUTUBE_COOKIES, 'utf-8');
    } catch (e) {
  ...
  if (cookiesFile) baseOptions.cookies = cookiesFile;

  let info;
  try {
    // Attempt 1: Standard clients (best quality)
    info = await youtubedl(videoId, baseOptions);
  } catch (err) {
    console.warn('[DownloadYes] Standard client blocked, falling back to mobile clients...', err.message);
    // Attempt 2: Fallback to Android/iOS AND skip HTML webpage download to bypass Vercel bot checks
    info = await youtubedl(videoId, { ...baseOptions, extractorArgs: 'youtube:player_client=android,ios;player_skip=webpage,configs' });
  }
...
async function getDownloadStream(videoId, options = {}) {
  ...
  const baseOptions = { dumpSingleJson: true, noWarnings: true, rmCacheDir: true, addHeader: [`X-Forwarded-For:${randomIp}`] };
  if (cookiesFile) baseOptions.cookies = cookiesFile;

  // Helper to fetch info with fallback
  let info, extractorArgs = null;
  try {
    info = await youtubedl(videoId, baseOptions);
  } catch (err) {
    extractorArgs = 'youtube:player_client=android,ios;player_skip=webpage,configs';
    info = await youtubedl(videoId, { ...baseOptions, extractorArgs });
  }
  ...
  if (options.type === 'mp3') {
    ...
    if (extractorArgs) ytdlOptions.extractorArgs = extractorArgs;
