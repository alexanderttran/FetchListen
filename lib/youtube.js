...
async function getAudioStream(videoId, cookiesContent, req, res) {
  ...
  const itag18 = formats.find(f => f.itag === 18);
  if (!itag18) {
    throw new Error('itag 18 format not available for this video');
  }

  // Get content length from the format if available
  const contentLength = itag18.content_length ? Number(itag18.content_length) : null;
  const mimeType = 'audio/mp4'; // itag 18 is video/mp4 but browsers accept audio/mp4 for audio
