...
async function getInnertube(cookiesContent) {
  ...
  console.log(`[getInnertube] Cookie check: raw length = ${rawCookie ? rawCookie.length : 0}, parsed length = ${cookie ? cookie.length : 0}`);

  if (cookie) {
    try {
      const keys = cookie.split(';').map(p => p.split('=')[0].trim()).filter(Boolean);
      console.log(`[getInnertube] Authenticated Session — cookie keys present: ${keys.join(', ')}`);
    } catch (e) {}
  ...
  console.log(`[getInnertube] Anonymous Session — no cookies provided`);
  if (!cachedYt) {
